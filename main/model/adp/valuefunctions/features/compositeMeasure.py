...
def composite_adopted_measure(terminal: Terminal, event: Events, current_batch_number: int, corridor: Optional[List[int]]):
    included_block_indices = get_block_indices(terminal, corridor)
    mmrule_score = MM_adopted_rule(terminal, included_block_indices)
    crl_score = crl(terminal, included_block_indices)
    return ALPHA * mmrule_score + (1-ALPHA) * crl_score


def composite_measure(terminal: Terminal, event: Events, current_batch_number: int, corridor: Optional[List[int]]):
    included_block_indices = get_block_indices(terminal, corridor)
    mmrule_score = MM_rule(terminal, included_block_indices)
    crl_score = crl(terminal, included_block_indices)
    return ALPHA * mmrule_score + (1-ALPHA) * crl_score


def crl(terminal: Terminal, included_block_indices: List[int]) -> float:
    ...
    for target_container in retrieval_order:
        if target_container not in removed_containers:
            ...
            for blocking_container in blocking_containers:
                ...
                total_reshuffles += int(not causes_no_additional_reshuffle(current_terminal, blocking_container, target_container_location, included_block_indices))
...
def causes_no_additional_reshuffle(terminal: Terminal, container: Container, to_exclude: StackTierLocation, included_block_indices: List[int]) -> bool:
    ...
    valid_stacks = get_valid_stacks(terminal, to_exclude, included_block_indices)
