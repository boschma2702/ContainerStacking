...
def get_valid_stacks(terminal: Terminal,
                     to_exclude: Optional[StackTierLocation],
                     included_blocks_indices: List[int],
                     container_label: int) -> List[Tuple[Stack, StackLocation]]:
    available_stacks = []
    for block_index in included_blocks_indices:
        block = terminal.blocks[block_index]
        if container_allowed_in_block(block, container_label):
...
def container_allowed_in_block(block, container_label):
    return container_label == 0 or container_label == None or container_label == block.designated
